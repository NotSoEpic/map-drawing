package wawa.mapwright.gui;

import net.minecraft.client.gui.GuiGraphics;
import net.minecraft.resources.ResourceLocation;
import wawa.mapwright.MapwrightClient;

public record GUIElementAtlases(
        ResourceLocation loc,
        int startX,
        int startY,
        int width,
        int height,
        int stampBagSaveScreenDimensions) {

    private static final ResourceLocation STAMP_BAG_SAVE_LOC = MapwrightClient.id("textures/gui/sprites/stamp_bag_screen.png");
    private static final int STAMP_BAG_SAVE_SCREEN_DIMENSIONS = 128;

    public static final GUIElementAtlases
            STAMP_BAG_SAVE = create(STAMP_BAG_SAVE_LOC, 0, 0, 128, 25, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
            STAMP_BAG_CLOSED = create(STAMP_BAG_SAVE_LOC, 96, 64, 16, 16, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
            STAMP_BAG_OPEN = create(STAMP_BAG_SAVE_LOC, 112, 64, 16, 16, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
            STAMP_BAG_CLOSED_HIGHLIGHT = create(STAMP_BAG_SAVE_LOC, 92, 80, 18, 18, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
            STAMP_BAG_OPEN_HIGHLIGHT = create(STAMP_BAG_SAVE_LOC, 110, 80, 18, 18, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
    STAMP_BAG_SAVE_CONFIRM = create(STAMP_BAG_SAVE_LOC, 64, 64, 16, 16, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
            STAMP_BAG_SAVE_CANCEL = create(STAMP_BAG_SAVE_LOC, 80, 64, 16, 16, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),

            STAMP_BAG_BROWSE_ENTRY = create(STAMP_BAG_SAVE_LOC, 0, 32, 128, 32, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
            STAMP_BAG_BROWSE_UP = create(STAMP_BAG_SAVE_LOC, 32, 64, 16, 16, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
            STAMP_BAG_BROWSE_DOWN = create(STAMP_BAG_SAVE_LOC, 48, 64, 16, 16, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
            STAMP_BAG_BROWSE_UNFAVORITE = create(STAMP_BAG_SAVE_LOC, 0, 64, 16, 16, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
            STAMP_BAG_BROWSE_FAVORITE = create(STAMP_BAG_SAVE_LOC, 0, 80, 16, 16, STAMP_BAG_SAVE_SCREEN_DIMENSIONS),
    STAMP_BAG_BROWSE_TRASH = create(STAMP_BAG_SAVE_LOC, 16, 64, 16, 16, STAMP_BAG_SAVE_SCREEN_DIMENSIONS);

    public static GUIElementAtlases create(final ResourceLocation loc, final int startX, final int startY, final int width, final int height, final int dimension) {
        return new GUIElementAtlases(loc, startX, startY, width, height, dimension);
    }

    public void render(final GuiGraphics guiGraphics, final int x, final int y) {
        guiGraphics.blit(this.loc, x, y, this.startX, this.startY, this.width(), this.height(), this.stampBagSaveScreenDimensions, this.stampBagSaveScreenDimensions);
    }
}
