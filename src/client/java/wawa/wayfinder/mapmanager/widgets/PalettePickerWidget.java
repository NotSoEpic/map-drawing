...
public class PalettePickerWidget extends AbstractWidget {
    ...
    private final ToolSelectionWidget parent;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.active && this.visible && this.isValidClickButton(button) && this.clicked(mouseX, mouseY)) {
            if (super.clicked(mouseX, mouseY)) {
                int index = getSelectedIndex(mouseX, mouseY);
                if (index != -1) {
                    ToolSelectionWidget.brush.setColorIndex(paletteIndices[index]);
                    Tool.set(ToolSelectionWidget.brush);
                }
